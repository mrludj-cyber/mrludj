...
**[중요 : 판례번호(ID) 수집 - 필수적 전수 조사(Exhaustive Search)]**
관련된 판례가 100개라면 100개 모두 리스트로 만들어줘. 요약하거나 선별하지 말고, 검색되는 모든 사건 번호를 빠짐없이 나열해.
내가 질문한 사항에 관련된 모든 판례 번호를 검색해서 리스트만 먼저 작성해.
...
4. **그룹화(Grouping)**: 판례가 너무 많을 경우, [유사 쟁점] 또는 [연도별]로 그룹화하여 가독성을 높이되, 판례번호(ID)는 절대 생략하지 마십시오.
