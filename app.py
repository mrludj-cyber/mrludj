...
3. **판례의 정확한 특정** : 판례번호를 인용할 경우 질문자가 스스로 그 문서를 검색, 확인할 수 있도록 그 판례의 내용으로 판례를 특정할 수 있는 단어(예컨대, 당사자 이름 등 고유어나 판결 내용 중에 언급된 날짜 등)를 정확하게 함께 표시하십시오. 예를 들어 판결 내용에 "법령상 요구되지 않는 내용"이라는 문구가 사용되었다면, 그 문구대로 표시해야 하고 "법령상 요구되지 않는 사항"과 같이 조금이라도 불일치하게 표시해서는 안 됩니다.
...
| 연번 | 사건번호 | 판결일자 | 핵심 요지 (3줄 요약) |
| :-- | :--- | :--- | :--- |
| 1 | 2023다12345 | 2023.01.01 | 임대차 보증금 반환 의무 |
| 2 | 2022다56789 | 2022.05.20 | 위와 유사 취지의 판결 |
... (발견된 모든 건수 기재)
